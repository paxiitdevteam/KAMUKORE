# French Pages Fix - Language Switcher

## Issue
French language switcher (FR button) was not working - clicking didn't translate the page.

## Root Cause
When components are loaded dynamically via JavaScript:
1. Components load after page loads
2. Language switcher event listeners are attached before components exist
...
Updated `main.js` to expose functions globally:
```javascript
...
Updated `components.js` to reattach event listeners after components load:
```javascript
setTimeout(function() {
...
Updated all pages to wait for components before initializing:
```javascript
setTimeout(function() {
